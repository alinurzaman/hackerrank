 ...
 */
func removeDuplicateValues(intSlice []int32) []int32 {
...
func climbingLeaderboard(ranked []int32, player []int32) []int32 {
	...
	var rank []int32
	listWithoutDuplicates := removeDuplicateValues(ranked)
	index := len(listWithoutDuplicates) - 1
	for i := 0; i < len(player); i++ {
		for j := index; j >= 0; j-- {
			if player[i] >= listWithoutDuplicates[0] {
				...
				rank = append(rank, int32(j)+2)
				index = j
				break
			} else if player[i] == listWithoutDuplicates[j] {
				rank = append(rank, int32(j)+1)
				index = j
				break
			}
		}
